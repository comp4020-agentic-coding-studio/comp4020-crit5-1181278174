...
### 还欠 director 一句话（正文按推荐值先写，你否决我就改）

- **栈**：模板自带的 vanilla TS + Vite + Canvas 2D。零转换成本，仓库现在已经 `pnpm check` 全绿，
  `CLAUDE.md` 里那五条 Facts that bite 全部是针对这个栈验过的。**推荐，除非否决按此执行。**
- **同屏只允许自己的一发子弹。** 强烈推荐：它让每一枪都是承诺、天然形成冷却而不用计时器，
  而且直接修好"死因不可归因"——场上只有一发子弹，所以死于子弹时你一定看得见是哪一发。
- **第 1 波用固定布局，第 2、3 波随机生成。** 随机生成拿走了编排教学时刻的能力（见 §4），
  固定第一张图的成本是一个字符串常量，买回来的是整个开场教学。
- **触屏**：pointer 事件统一 mouse/touch/pen，但触屏上"移动光标"和"点击"是同一个动作。
  建议沿用 C4 的 TAP 阈值做法（10 px / 250 ms 内抬起 = 开火，否则是转向拖拽），排在核心之后做。
